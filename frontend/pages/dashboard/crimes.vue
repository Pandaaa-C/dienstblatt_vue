<template>
    <div class="page">
        ...
        <h2>
            Schwarzgeld Gesamtsumme: <span>${{ getMoneyString(Number(blackMoneyAmount.toFixed(0))) }}</span>
        </h2>
        <div class="list">
            <div class="head">
                <p class="name">Name</p>
            ...
            <div class="item" v-for="item in crimes" :key="item._id">
                ...
                <p class="black-money">
                    P: ${{ getMoneyString(item.blackMoney.person) }} | F: ${{ getMoneyString(item.blackMoney.vehicle) }} | H: ${{
                        getMoneyString(item.blackMoney.house)
                    }}
                </p>
                ...
                <div class="illegal-items"><input type="checkbox" v-model="item.illegalItems" onclick="return false;" /></div>
                <p :class="`proof-item link ${item.proofs.proofOne.length < 1 ? 'empty' : ''}`" @click="openLink(item.proofs.proofOne)">
                    Beweis 1
                </p>
                <p :class="`proof-item link ${item.proofs.proofTwo.length < 1 ? 'empty' : ''}`" @click="openLink(item.proofs.proofTwo)">
                    Beweis 2
                </p>
                <p :class="`proof-item link ${item.proofs.proofThree.length < 1 ? 'empty' : ''}`" @click="openLink(item.proofs.proofThree)">
                    Beweis 3
                </p>
                <div class="open"><input type="checkbox" v-model="item.finished" @click="updateCrime(item._id, item.finished)" /></div>
            </div>
        </div>
    </div>
</template>
    ...
    return searchInput.value.length < 1
        ...
        : crimeStore.crimes.filter(
              (x: any) =>
                  ...
                  x.faction.toLowerCase().includes(searchInput.value.toLowerCase()) ||
                  x.location.toLowerCase().includes(searchInput.value.toLowerCase()),
          );
});
...
const addCrime = async (): Promise<void> => {
    ...
    if (response.message.toLowerCase().includes('erfolgreich')) {
        ...
        team = '';
        location.value = '';
        blackMoneyPerson.value = undefined;
        blackMoneyVehicle.value = undefined;
        blackMoneyHouse.value = undefined;
        bnc.value = false;
        camper.value = '';
        mord.value = false;
        illegalItems.value = false;
        proof1.value = '';
        proof2.value = '';
        proof3.value = '';
    }
...
const openLink = (link: string): void => {
    if (link === undefined || link.length < 1) {
        componentStore.sendNotification('Dieser Link ist ungültig!');
        return;
    }
...
const selectTeam = (index: number): void => {
    team = teams.value[index] as string;
};
</script>
