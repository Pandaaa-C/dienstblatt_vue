...
const liabilities = computed(() => {
    return searchInput.length < 1
        ...
        : liabilityStore.reductions.filter(
              (x: any) =>
                  x.name.toLowerCase().includes(searchInput.toLowerCase()) || x.reduction.toLowerCase().includes(searchInput.toLowerCase()),
          );
});
...
const addReduction = async (): Promise<void> => {
    ...
    const response: { message: string } = (
        await $fetch<{ message: string }>(apiUrl + '/liability/addReduction', {
            method: 'POST',
            body: {
                name: name,
                jailtime: jailTime,
